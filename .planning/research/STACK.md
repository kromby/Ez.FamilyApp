...
**Confidence:** MEDIUM-HIGH — core choices verified via official docs and multiple sources; exact minor versions should be confirmed at install time via `npx expo install`
...
**Why Expo over bare React Native:** For a family project with no custom native modules, Expo managed workflow eliminates the need to deal with Xcode/Gradle. EAS Build handles App Store / Play Store distribution without a Mac CI. SDK 55 is current stable as of February 2026.

**Why Expo Router over React Navigation:** New Expo projects ship with Expo Router by default. For a mobile-only app with a simple tab structure (channels, tasks, check-in, profile), file-based routing is straightforward and eliminates manual navigator setup. Expo Router wraps React Navigation — React Navigation APIs are available if needed.
...
**Why Supabase over Firebase:** PostgreSQL gives full relational querying power, especially for tasks and family membership. RLS policies enforce data isolation at the DB level — no family can see another family's data. Supabase is open-source and the JS SDK is isomorphic, with an official Expo quickstart guide maintained by Supabase.

**Why not a dedicated chat service (Stream, Sendbird, Pusher):** These are paid services with per-MAU pricing. For 5-10 users, Supabase Realtime Broadcast is more than sufficient, avoids a third-party dependency, and keeps all data in one place.
...
**Why not Redux Toolkit:** Redux is the right answer for large teams. For a family app with one developer, Zustand removes 80% of the ceremony with equivalent capability.

**Why both TanStack Query and Zustand:** These are complementary, not competing. TanStack Query owns server-synchronized state. Zustand owns local transient state. Using TanStack Query alone for all state creates awkward "fake queries." Using Zustand alone for server state means rebuilding cache invalidation.
...
**NativeWind v4 vs v5 caveat:** As of April 2026, NativeWind v5 targets Expo SDK 55 but is still pre-release. NativeWind v4 was stable against SDK 52-54. Options: (a) pin to SDK 54 for the stability pairing, (b) use SDK 55 + wait for NativeWind v5 stable, (c) use plain `StyleSheet` for v1 and add NativeWind once the pairing is confirmed. **Recommendation: use plain `StyleSheet` for v1 to avoid a compatibility bet. Add NativeWind in v2 once its SDK 55 support stabilizes.** Confidence: MEDIUM (needs validation against current NativeWind releases at project start).
...
This stack maps directly to the app's requirements:
